...
export function CartClient() {
  ...
  return <div className="space-y-5">{items.map((item) => <div key={item.product_id} className="flex gap-4 rounded-2xl border border-slate-200 bg-white p-4">{item.image_url ? <Image src={item.image_url} alt="" width={80} height={80} className="h-20 w-20 rounded-lg object-contain" /> : null}<div className="min-w-0 flex-1"><Link href={`/produkt/${item.slug}`} className="font-bold text-slate-900 hover:text-brand-700">{item.title}</Link><div className="mt-1 text-sm text-slate-500">{item.quantity} ks × {item.final_price.toFixed(2)} €</div></div><button type="button" onClick={() => remove(item.product_id)} className="text-xs font-semibold text-rose-600">Odstrániť</button></div>)}<div className="flex items-center justify-between rounded-2xl bg-slate-900 p-5 text-white"><span className="font-bold">Medzisúčet</span><span className="text-xl font-black">{total.toFixed(2)} €</span></div><form onSubmit={createOrder} className="space-y-3 rounded-2xl border border-slate-200 bg-white p-5"><h2 className="text-lg font-black">Dokončiť objednávku</h2><input name="customerName" required placeholder="Meno a priezvisko" className="w-full rounded-lg border border-slate-200 px-3 py-2" /><input name="customerEmail" required type="email" placeholder="E-mail" className="w-full rounded-lg border border-slate-200 px-3 py-2" /><input name="customerPhone" placeholder="Telefón (voliteľné)" className="w-full rounded-lg border border-slate-200 px-3 py-2" /><input name="street" required placeholder="Ulica a číslo" className="w-full rounded-lg border border-slate-200 px-3 py-2" /><div className="grid grid-cols-2 gap-3"><input name="city" required placeholder="Mesto" className="w-full rounded-lg border border-slate-200 px-3 py-2" /><input name="postalCode" required placeholder="PSČ" className="w-full rounded-lg border border-slate-200 px-3 py-2" /></div><input name="country" defaultValue="Slovensko" required placeholder="Krajina" className="w-full rounded-lg border border-slate-200 px-3 py-2" />{error ? <p className="text-sm text-rose-600">{error}</p> : null}<button disabled={submitting} className="w-full rounded-xl bg-brand-600 px-5 py-3 font-bold text-white disabled:opacity-60">{submitting ? 'Spracúvam…' : 'Vytvoriť objednávku'}</button></form></div>;
}
