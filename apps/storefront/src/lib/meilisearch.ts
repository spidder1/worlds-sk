...
export async function searchMeilisearch(query: string, limit = 1000): Promise<{ ids: string[]; estimatedTotalHits: number } | null> {
  ...
  try {
    const response = await fetch(`${host}/indexes/${encodeURIComponent(index)}/search`, { method: 'POST', headers: { authorization: `Bearer ${apiKey}`, 'content-type': 'application/json' }, body: JSON.stringify({ q: query, limit: Math.min(1000, Math.max(1, limit)), attributesToRetrieve: ['id'] }), signal: AbortSignal.timeout(2500), cache: 'no-store' });
