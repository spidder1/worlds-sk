...
export default async function AdminProducts({ searchParams }: { searchParams: Promise<{ q?: string; page?: string; saved?: string }> }) {
  ...
  const args = q ? [`%${q}%`] : [];
  const products = await queryNeon<{ id: string; title: string; sku: string; brand: string; category_slug: string; final_price: string }>(`SELECT id, title, sku, brand, category_slug, final_price::text FROM products ${where} ORDER BY updated_at DESC LIMIT $${args.length + 1} OFFSET $${args.length + 2}`, [...args, limit, offset]);
