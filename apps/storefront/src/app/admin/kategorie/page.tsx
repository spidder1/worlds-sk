...
export default async function AdminCategories() {
  ...
  const categories = await queryNeon<{ slug: string; name: string; parent_slug: string | null; level: number; active: boolean; count: string }>(`WITH RECURSIVE category_tree AS (
  ...
  SELECT tree.slug, tree.name, tree.parent_slug, tree.level, tree.active, COUNT(p.id)::text AS count
    FROM category_tree tree LEFT JOIN products p ON p.category_slug = tree.slug
   GROUP BY tree.slug, tree.name, tree.parent_slug, tree.level, tree.active, tree.sort_path
