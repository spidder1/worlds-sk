      ...
      <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <div className="mt-3 overflow-x-auto">
          <table className="w-full min-w-[720px] text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {lifecycle.map((row) => <tr key={`${row.product_id}-${row.changed_at}`}><td className="py-2"><div className="font-medium">{row.product_name || row.product_id}</div><div className="text-xs text-slate-500">{row.product_id}</div></td><td>{row.old_status || '—'} → {row.new_status}</td><td>{row.reason || '—'}</td><td className="text-slate-500">{new Date(row.changed_at).toLocaleString('sk-SK')}</td></tr>)}
            </tbody>
          </table>
      ...
      <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <div className="mt-3 overflow-x-auto">
          <table className="w-full min-w-[720px] text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {issues.map((row, index) => <tr key={`${row.source_key}-${row.issue_code}-${row.created_at}-${index}`}><td className="py-2">{row.source_key || '—'}</td><td>{row.severity}</td><td>{row.issue_code}</td><td>{row.message}</td><td className="text-slate-500">{new Date(row.created_at).toLocaleString('sk-SK')}</td></tr>)}
            </tbody>
          </table>
