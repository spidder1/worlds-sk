...
type SettingRow = { key: string; value: { value?: number | boolean | string | null } };
...
export default async function AdminSettingsPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  const [settings, rules, transportMethods] = await Promise.all([
    ...
    queryNeon<TransportRow>('SELECT code, name, type_code FROM supplier_transport_methods WHERE active = true ORDER BY name'),
  ]);
  ...
  return <div className="mx-auto max-w-4xl space-y-6">
    ...
    <form action={updatePricingSettings} className="space-y-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <label className="text-sm font-semibold sm:col-span-2">Predvolený dopravca eD<select name="transport_code" defaultValue={String(values.get('orders.default_transport_code') ?? '')} className="mt-1 w-full rounded-lg border p-2 font-normal"><option value="">Bez výberu (TransportCode 0)</option>{transportMethods.map((method) => <option key={method.code} value={method.code}>{method.name} ({method.code})</option>)}</select><span className="mt-1 block text-xs font-normal text-slate-500">Zoznam sa obnovuje workflowom eD transport dictionary sync.</span></label>
      </div>
