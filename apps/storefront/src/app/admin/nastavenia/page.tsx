...
type SettingRow = { key: string; value: { value?: number } };
...
export default async function AdminSettingsPage() {
  ...
  const [settings, rules] = await Promise.all([
    queryNeon<SettingRow>(`SELECT key, value FROM store_settings WHERE key IN ('pricing.vat_rate', 'feed.minimum_cost_eur')`),
  ...
  return <div className="mx-auto max-w-4xl space-y-6">
    ...
    <form action={updatePricingSettings} className="space-y-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="text-sm font-semibold">DPH (%)<input name="vat_rate" type="number" min="0" max="100" step="0.01" defaultValue={values.get('pricing.vat_rate') ?? 20} className="mt-1 w-full rounded-lg border p-2 font-normal" /></label>
        <label className="text-sm font-semibold">Minimálna nákupná cena feedu (€)<input name="minimum_cost_eur" type="number" min="0" step="0.01" defaultValue={values.get('feed.minimum_cost_eur') ?? 0} className="mt-1 w-full rounded-lg border p-2 font-normal" /></label>
      </div>
