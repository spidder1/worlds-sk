    ...
    <div className="mt-6 space-y-4">
      {orders.map((order) => <article key={order.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div><h3 className="font-bold"><a href={`/api/orders/${order.id}/invoice`} className="hover:text-brand-700">{order.order_number}</a></h3><p className="mt-1 text-sm text-slate-600">{order.customer_name} · {order.customer_email}</p><p className="text-xs text-slate-500">{order.customer_type === 'LEGAL' ? `Právnická osoba · IČO ${order.customer_ico || '—'} · DIČ ${order.customer_dic || '—'}` : 'Súkromná osoba'}</p></div>
