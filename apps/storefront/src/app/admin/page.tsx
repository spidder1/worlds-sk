...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  const [products, priced, manufacturers, batches] = await Promise.all([
  ...
  const cards = [['Produkty', products[0]?.count || '0'], ['S platnou cenou', priced[0]?.count || '0'], ['Výrobcovia', manufacturers[0]?.count || '0']];
  return <div><div className="flex items-end justify-between"><div><h2 className="text-2xl font-bold">Prehľad katalógu</h2><p className="mt-1 text-sm text-slate-600">Rýchla kontrola stavu importu a verejného katalógu.</p></div></div><div className="mt-6 grid gap-4 md:grid-cols-3">{cards.map(([label, value]) => <div key={label} className="rounded-2xl border border-slate-200 bg-white p-5"><p className="text-sm text-slate-500">{label}</p><p className="mt-2 text-3xl font-bold">{value}</p></div>)}</div><section className="mt-8 rounded-2xl border border-slate-200 bg-white p-5"><h3 className="font-semibold">Posledné importy</h3><div className="mt-3 divide-y divide-slate-100">{batches.map((batch, index) => <div key={`${batch.started_at}-${index}`} className="flex flex-wrap justify-between gap-2 py-3 text-sm"><span>{batch.mode}</span><span className="text-slate-500">{batch.status} · {new Date(batch.started_at).toLocaleString('sk-SK')}</span></div>)}</div></section></div>;
}
