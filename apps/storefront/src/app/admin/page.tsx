...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  const cards = [['Produkty', products[0]?.count || '0'], ['S platnou cenou', priced[0]?.count || '0'], ['Skladom', inStock[0]?.count || '0'], ['Priemerná kvalita', `${quality[0]?.average || '0'}/100`], ['Na kontrolu', reviewQueue[0]?.count || '0'], ['Výrobcovia', manufacturers[0]?.count || '0']];
