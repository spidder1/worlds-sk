...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const orders = await pool.query(`SELECT order_number, customer_name, customer_email, customer_ico, customer_dic, shipping_address, subtotal, total, currency, created_at FROM orders WHERE id = $1 LIMIT 1`, [id]);
  ...
  if (order.customer_ico || order.customer_dic) document.text(`IČO: ${order.customer_ico || '—'} · DIČ: ${order.customer_dic || '—'}`);
