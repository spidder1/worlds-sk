...
const checks = {
  ...
  localization_missing: `SELECT COUNT(*)::int AS count FROM products p WHERE NOT EXISTS (SELECT 1 FROM product_localizations pl WHERE pl.product_id = p.id AND pl.locale = 'sk')`,
  ...
  media_link_missing: `SELECT COUNT(*)::int AS count FROM product_media pm WHERE pm.media_asset_id IS NULL`,
...
try {
  ...
  console.log(JSON.stringify({ checkedAt: new Date().toISOString(), results, healthy: critical.length === 0 }, null, 2));
